...
public class JavacResolution {
    ...
    public Map<JCTree, JCTree> resolveMethodMember(JavacNode node) {
        ...
        try {
            ...
            //似乎只是解析得到了包裹方法调用语句的EnclosingElement?
            while (!stack.isEmpty()) {
                //stack.pop().accept(finder);
                JCTree top = stack.pop();
                //System.out.println("顶部元素："+top);
                top.accept(finder);
                //System.out.println("查找测试："+finder.copyAt);
            }
            //System.out.println("解析测试："+finder.copyAt());
            ...
            //System.out.println("环境env测试："+finder.get());
            try {
...
//            AssertionLogger.assertLog("member enter failed.", ignore);
    ...
    private void attrib(JCTree tree, Env<AttrContext> env) {
        //System.out.println("环境测试："+env.enclClass);
        try {
